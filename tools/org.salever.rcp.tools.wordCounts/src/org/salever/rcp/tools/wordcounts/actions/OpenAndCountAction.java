...
import org.salever.rcp.tools.wordcounts.editors.WordCountEditor;

/**
 * @author
 * 
 */
public class OpenAndCountAction extends Action {

	private static final String ID = "org.salever.rcp.tech.chapter6.txtedit.action.OpenAndCountAction"; //$NON-NLS-1$

	private IWorkbenchWindow window;

	public OpenAndCountAction(IWorkbenchWindow window) {
		this.window = window;
		setText(Messages.OpenAndCountAction_1);
		setId(ID);
	...
	public void run() {
		...
		if (path != null) {
			...
			try {
				page.openEditor(input, WordCountEditor.ID);
