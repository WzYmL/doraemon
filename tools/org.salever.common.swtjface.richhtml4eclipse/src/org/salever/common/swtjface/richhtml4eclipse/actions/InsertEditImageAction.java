...
import org.salever.common.swtjface.richhtml4eclipse.dialogs.ImageDialog;
...
public class InsertEditImageAction extends Action implements Listener {
	...
	public InsertEditImageAction(HtmlComposer composer) {
		super(Messages.InsertEditImageAction_0, IAction.AS_CHECK_BOX);
		setImageDescriptor(ActionUtil.getImageDescriptor("images/image.gif")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public void run() {
		...
		if (dialog.open() == IDialogConstants.OK_ID) {
			this.composer.execute(JavaScriptCommands.INSERT_IMAGE(
					dialog.getSource(), dialog.getAlt(), dialog.getBorder(),
					dialog.getHspace(), dialog.getVspace(), dialog.getWidth(),
					dialog.getHeight(), dialog.getAlign()));
		}
	}
