package org.salever.rcp.dbsystem.client.ui.dialogs;
...
public class LoginDialog extends TitleAreaDialog {
	...
	protected void buttonPressed(int buttonId) {
		...
		if (buttonId == IDialogConstants.OK_ID) {
			...
			try {
				DBCoreActivator activator = DBCoreActivator.getDefault();
				activator.initializeApplicationContext();
				AdminService service = activator.getBean(AdminService.class);
				model = service.login(username.getText(), password.getText());
			} catch (Exception e) {
	...
	protected Control createDialogArea(Composite parent) {
		...
		btnNewButton_1.setText("数据库配置"); //$NON-NLS-1$
	...
	protected void configDataServer() {
		PreferencesUtil.createPreferenceDialogOn(getShell(),
				DBCoreActivator.PREF_ID,
				new String[] { DBCoreActivator.PREF_ID }, //$NON-NLS-1$
				null).open();
