 ...
 *******************************************************************************/
package org.salever.osgi.http.security;
...
import org.salever.osgi.http.security.util.ITrackingConstants;
...
public class TrackingConfigurationServlet extends HttpServlet {
	...
	public TrackingConfigurationServlet(String servletAlias, String id,
		...
		if (delayString != null) {
			...
			if (delay == 0) {
				response.sendError(HttpServletResponse.SC_NOT_ACCEPTABLE,
						ILLEGAL_DELAY_VALUE + delayString);
				return;
			}
			updateDelay(delay);
		}
	...
	private int getCurrentDelay() {
		try {
			Configuration config = configAdmin.getConfiguration(
					ITrackingConstants.TRACKING_PID,
					ITrackingConstants.TRACKING_LOCATION);
			Dictionary properties = config.getProperties();
			int delay = ITrackingConstants.TRACKING_DELAY_DEFAULT;
			if (properties != null) {
				Integer delayInteger = (Integer) properties
						.get(ITrackingConstants.TRACKING_DELAY_PROPERTY);
				delay = delayInteger.intValue();
			}
			return delay;
		} catch (IOException e) {
			System.err.println("Unable to get tracking configuration: " + e);
		}
		return 0;
	}
	...
	private void updateDelay(int delay) {
		try {
			...
			properties.put(ITrackingConstants.TRACKING_DELAY_PROPERTY,
					delayInteger);
			config.update(properties);
		} catch (IOException e) {
			System.err.println(this
					+ "Unable to update tracking configuration: " + e);
		}
	}
}
